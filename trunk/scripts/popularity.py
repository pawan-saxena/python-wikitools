...
def main():
	...
	wiki = Wiki.Wiki()
	wiki.login("user")
	db = MySQLdb.connect(host="localhost", user="user", passwd="paswd", use_unicode=True)
	cursor = db.cursor()
	...
	for type in articletypes.keys():
		...
		for page in catpage.getAllMembersGen():
			...
			#query = 'SELECT * FROM `'+projectabbrv+'` WHERE title = "'+realtitle.title.decode('utf-8')+'"'
			...
			#query = "INSERT INTO `"+projectabbrv+"` (title, hits, assess) VALUES( '"+realtitle.title+"', "+hits+", '"+type+"' )"
	...
	target = Wiki.Page(wiki, listpage)
